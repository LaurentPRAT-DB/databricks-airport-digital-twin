...
class TestDataIntegrity:
    ...
    def test_O03_all_arrivals_complete_lifecycle(self, traces, sim):
        ...
        if arrivals and defects:
            ...
            assert rate < 0.40, f"O03 [{airport}]: {len(defects)}/{len(arrivals)} arrivals incomplete:\n" + "\n".join(defects[:5])
    ...
    def test_O05_smooth_altitude_during_approach(self, traces, sim):
        ...
        Snapshot interval is 30s with 2s ticks, so climb at 25 ft/s produces
        up to 750ft per recorded snapshot — threshold is 800ft to allow margin.
        Go-around climbs from low altitude (< 1000ft) are expected.
        """
        _, _, airport = sim
        defects = []
        for icao24, trace in traces.items():
