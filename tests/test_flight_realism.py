...
class TestGateTurnaroundTiming:
    ...
    def test_narrow_body_turnaround_not_under_10_minutes(self):
        ...
        Real turnaround gate time is ~27 min (45 total minus taxi/pushback phases).
        With airline/weather factors and jitter, minimum is ~22 min.
        """
        flight = _create_new_flight(
    ...
    def test_wide_body_stays_parked_longer(self):
        ...
        Real-world turnaround times (no speedup):
        - Narrow-body (A320) ~27 min gate time (45 total - taxi/pushback)
        - Wide-body (B777) ~68 min gate time (90 total - taxi/pushback)
        At 30 min, A320 should have pushed back but B777 should still be parked.
        """
        # A320 at gate for 30 min → should push back (~27 min gate time)
        ...
        # B777 at gate for 30 min → should still be parked (~68 min gate time)
        ...
        # B777 at 30 min should still be parked (wide-body: ~68 min gate time)
        _update_flight_state(b777, 1.0)
