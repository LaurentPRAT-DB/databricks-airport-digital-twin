...
from src.ml.turnaround_features import (
    TurnaroundBoardFeatureSet,
    TurnaroundCoarseFeatureSet,
...
def _make_full_fs(**overrides) -> TurnaroundFeatureSet:
    """Create TurnaroundFeatureSet with sensible defaults for all fields."""
    defaults = dict(
    ...
    return TurnaroundFeatureSet(**defaults)


def _make_coarse_fs(**overrides) -> TurnaroundCoarseFeatureSet:
    """Create TurnaroundCoarseFeatureSet with sensible defaults for all fields."""
    defaults = dict(
        aircraft_category="narrow",
        airline_code="UAL",
        scheduled_departure_hour=16,
    ...
    defaults.update(overrides)
    return TurnaroundCoarseFeatureSet(**defaults)
...
class TestTurnaroundFeatureSet:
...
class TestExtractTrainingData:
    ...
    def test_features_have_all_fields(self):
        ...
        for field in TurnaroundFeatureSet.__dataclass_fields__:
...
class TestTurnaroundPredictorFallback:
    def test_fallback_when_untrained(self):
        predictor = TurnaroundPredictor(airport_code="KSFO")
...
class TestTurnaroundPredictorOBT:
    def test_predict_pushback_time_adds_duration(self):
        predictor = TurnaroundPredictor()
        fs = _make_full_fs()
        parked_ts = 1000000.0
        obt = predictor.predict_pushback_time(parked_ts, fs)
...
def _make_sample_features(n: int = 100) -> tuple[list[TurnaroundFeatureSet], list[float]]:
...
class TestTurnaroundPredictorTraining:
    def test_train_and_predict(self):
        predictor = TurnaroundPredictor(airport_code="TEST")
...
class TestOBTModelPersistence:
    def test_save_and_load(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            path = Path(tmpdir) / "turnaround_model.pkl"
            predictor.save(path)
            assert path.exists()

            loaded = TurnaroundPredictor(airport_code="TEST")
...
class TestOBTRegistryIntegration:
    ...
    def test_registry_includes_obt_model(self, mock_loader_cls):
        ...
        assert "turnaround" in models
        assert isinstance(models["turnaround"], TurnaroundPredictor)
        assert models["turnaround"].airport_code == "KTEST"

    @patch("src.ml.registry.AirportProfileLoader")
    def test_registry_retrain_includes_turnaround(self, mock_loader_cls):
        mock_loader = mock_loader_cls.return_value
        mock_loader.get_profile.return_value = None
...
class TestOBTTrainingPipeline:
    ...
    def test_multi_airport_training(self):
        ...
        predictor = TurnaroundPredictor(airport_code="GLOBAL")
...
class TestTurnaroundCoarseFeatureSet:
    ...
    def test_full_to_coarse_projection(self):
        ...
        assert isinstance(coarse, TurnaroundCoarseFeatureSet)
...
class TestTurnaroundCoarsePredictorFallback:
    def test_fallback_when_untrained(self):
        predictor = TurnaroundCoarsePredictor(airport_code="KSFO")
        assert not predictor.is_trained
    ...
    def test_fallback_wide_body(self):
        predictor = TurnaroundCoarsePredictor()
        fs = _make_coarse_fs(
...
class TestTurnaroundCoarsePredictorTraining:
    def test_train_and_predict(self):
        features, targets = _make_sample_features(200)
        coarse_features = [f.to_coarse() for f in features]

        predictor = TurnaroundCoarsePredictor(airport_code="TEST")
    ...
    def test_save_and_load(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            loaded = TurnaroundCoarsePredictor(airport_code="TEST")
            assert loaded.load(path) is True
            assert loaded.is_trained
...
class TestTwoStageTurnaroundPredictor:
    def test_train_both_stages(self):
        features, targets = _make_sample_features(300)

        ts = TwoStageTurnaroundPredictor(airport_code="TEST")
    ...
    def test_save_and_load_both(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            loaded = TwoStageTurnaroundPredictor(airport_code="TEST")
    ...
    def test_predict_pushback_time_timestamps(self):
        features, targets = _make_sample_features(200)
        ts = TwoStageTurnaroundPredictor(airport_code="TEST")
        ts.train(features, targets)
        ...
        obt_t90 = ts.predict_pushback_time_t90(scheduled_dep, features[0].to_coarse())
        obt_tpark = ts.predict_pushback_time_tpark(parked_time, features[0])
...
class TestTwoStageOnRealData:
    def test_train_and_compare(self):
        ...
        ts = TwoStageTurnaroundPredictor(airport_code="KSFO")
        result = ts.train(features, targets)
        assert result["coarse"]["status"] == "trained"
        assert result["refined"]["status"] == "trained"
...
class TestCalibratedModelQuality:
    ...
    def trained_model(self):
        ...
        ts = TwoStageTurnaroundPredictor(airport_code="GLOBAL")
...
class TestCalibratedCrossValidation:
    ...
    def test_leave_one_airport_out(self):
        ...
        for held_out_airport in test_airports:
            ...
            predictor = TurnaroundPredictor(airport_code="CV")
...
class TestOBTEdgeCases:
    ...
    def test_two_stage_untrained_uses_fallback(self):
        ...
        ts = TwoStageTurnaroundPredictor(airport_code="EMPTY")
    ...
    def test_obt_timestamp_ordering(self):
        """predict_pushback_time should always return a time after parked_time."""
        predictor = TurnaroundPredictor(airport_code="TEST")
        features, targets = _make_sample_features(200)
        predictor.train(features, targets)
...
class TestScheduledBufferFeature:
    def test_buffer_in_feature_set(self):
        """scheduled_buffer_min should be present in TurnaroundFeatureSet."""
        fs = _make_full_fs(scheduled_buffer_min=75.0)
        assert fs.scheduled_buffer_min == 75.0

    def test_buffer_in_coarse_feature_set(self):
        """scheduled_buffer_min should also be present in TurnaroundCoarseFeatureSet."""
...
class TestTBoardPredictor:
    def test_board_feature_set_creation(self):
        ...
        assert isinstance(board_fs, TurnaroundBoardFeatureSet)
    ...
    def test_board_predictor_train_and_predict(self):
        ...
        predictor = TurnaroundBoardPredictor(airport_code="TEST")
    ...
    def test_boarding_threshold(self):
        assert TurnaroundBoardPredictor.BOARDING_THRESHOLD == 0.70
...
class TestACDMAdapter:
    def test_valid_record_conversion(self):
        ...
        feat, target = result
        assert isinstance(feat, TurnaroundFeatureSet)
