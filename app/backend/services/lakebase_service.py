...
class LakebaseService:
    ...
    def get_trajectory(
        ...
        """
        Fetch trajectory history for a specific flight.
            ...
            with self._get_connection() as conn:
                with conn.cursor(cursor_factory=RealDictCursor) as cur:
                    ...
                    if not cur.fetchone()["exists"]:
                        logger.debug("flight_positions_history table not found")
                        return None
                    ...
                    cur.execute(
                        ...
                        LIMIT %s
                        """,
                        (icao24, str(minutes), limit),
                    )
                    rows = cur.fetchall()
                    logger.info(f"Trajectory for {icao24}: {len(rows)} positions")
                    return [dict(row) for row in rows]
