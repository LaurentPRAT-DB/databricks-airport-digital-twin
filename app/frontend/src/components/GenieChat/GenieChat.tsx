...
interface GenieChatProps {
  ...
  onClose?: () => void;
  /** Compact mode — show only the input field, no conversation history panel */
  compact?: boolean;
}

export default function GenieChat({ hideFab, externalOpen, onClose, compact }: GenieChatProps = {}) {
  ...
  // Compact mode: just the input bar, no full panel
  if (compact && isOpen) {
    return (
      <div className="fixed inset-0 z-[1100] flex flex-col bg-black/50" onClick={() => setIsOpen(false)}>
        <div className="flex-1" />
        <div
          className="bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-700 p-3 safe-area-pad-bottom"
          onClick={(e) => e.stopPropagation()}
        >
          {/* Latest response (if any) */}
          {messages.length > 0 && (
            <div className="mb-2 px-1 max-h-40 overflow-y-auto">
