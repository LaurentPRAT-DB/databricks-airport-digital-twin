...
describe('map3d-calculations', () => {
  ...
  describe('headingToRotation', () => {
    // Formula: -heading * PI / 180
    // Models face -Z (nose toward -Z = north); heading 0 needs no rotation.
    it('converts north (0°) to 0 rotation', () => {
    ...
    it('converts south (180°) to -180° rotation', () => {
      const rot = headingToRotation(180);
      expect(rot).toBeCloseTo(-Math.PI, 5);
    });

    it('converts west (270°) to -270° rotation', () => {
      const rot = headingToRotation(270);
      expect(rot).toBeCloseTo(-3 * Math.PI / 2, 5);
    });
