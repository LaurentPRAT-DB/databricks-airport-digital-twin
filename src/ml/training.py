...
def train_turnaround_model(
    ...
    """
    from src.ml.turnaround_features import extract_training_data, TurnaroundFeatureSet
    from src.ml.turnaround_model import TurnaroundPredictor, _dict_to_feature_set

    if experiment_name is None:
        experiment_name = f"airport_models/{airport_code}/turnaround_model"
    ...
    # Train
    predictor = TurnaroundPredictor(airport_code=airport_code, airport_profile=airport_profile)
    ...
    if model_output_path is None:
        ...
        model_output_path = str(
            model_dir / f"turnaround_model_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pkl"
        )
...
def train_obt_model(
    ...
    """Train the OBT (Off-Block Time) prediction model from simulation data.
    ...
    samples = extract_obt_training_data(sim_json_path)
    if not samples:
        return {"status": "no_data", "n_samples": 0}
