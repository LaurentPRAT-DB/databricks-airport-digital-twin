"""Feature extraction for turnaround duration prediction.
...
feature vectors for turnaround duration prediction (AIBT → AOBT).
"""
...
@dataclass
class TurnaroundCoarseFeatureSet:
    """Feature vector for T-90 (pre-arrival) turnaround prediction.
...
@dataclass
class TurnaroundFeatureSet:
    """Feature vector for T-park (refined) turnaround duration prediction.
    ...
    def to_coarse(self) -> TurnaroundCoarseFeatureSet:
    ...
    def to_board(
        ...
        return TurnaroundBoardFeatureSet(
            **{k: getattr(self, k) for k in TurnaroundFeatureSet.__dataclass_fields__},
...
@dataclass
class TurnaroundBoardFeatureSet(TurnaroundFeatureSet):
    """Feature vector for T-board (boarding start) turnaround prediction.
...
def extract_training_data(sim_json_path: str | Path) -> List[Dict[str, Any]]:
    ...
    Returns:
        List of dicts, each with 'features' (TurnaroundFeatureSet as dict) and
    ...
    logger.info(
        f"Extracted {len(results)} Turnaround training samples from {path.name} "
...
def extract_training_data_from_recording(recording_data: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Extract Turnaround training features from enriched recorded OpenSky data.
    ...
    logger.info(
        "Extracted %d Turnaround training samples from recorded data (airport=%s)",
